...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		// Título
		{
			...
			$this->Ln(20);
			$this->Cell(90, 10, utf8_decode('Nombre de la Pista'), 1, 0, 'C');
			$this->Cell(25, 10, utf8_decode('Tipo'), 1, 0, 'C');


			$this->Cell(70, 10, utf8_decode('Municipio'), 1, 0, 'C');
			$this->Cell(25, 10, utf8_decode('Teléfono'), 1, 0, 'C');
			$this->Cell(23, 10, utf8_decode('Precio/h'), 1, 0, 'C');
			$this->Cell(25, 10, utf8_decode('Apertura'), 1, 0, 'C');
			$this->Cell(20, 10, utf8_decode('Cierre'), 1, 0, 'C');
			$this->Ln();
		}
	}
...
if ($stmt = mysqli_prepare($c2, "SELECT p.id_pista, p.tipo_pista, p.nombre_pista, p.municipio, p.telefono_contacto, precio_hora, nombre_empresa, validacion, p.hora_apertura, p.hora_cierre 
    FROM pistas p, empresas e 
    WHERE e.id_empresa = p.id_empresa 
    AND e.id_empresa = ? 
    AND (validacion = 'ACTIVA' OR validacion = 'INACTIVA') 
    ORDER BY CASE 
        WHEN validacion = 'ACTIVA' THEN 0 
        WHEN validacion = 'INACTIVA' THEN 1 
        ELSE 2 
    END, p.nombre_pista ASC")) {
} else {
    echo mysqli_error($c2);
}

if (mysqli_stmt_bind_param($stmt, "s", $idempresa)) {
} else {
	echo mysqli_error($c2);
}
...
if (mysqli_stmt_bind_result($stmt, $id, $tipo, $nombre, $municipio, $telefono, $precio, $rnomempresa,$validacion, $rhoraper, $rhoracier)) {
} else {
	echo mysqli_error($c2);
}
// ...
while (mysqli_stmt_fetch($stmt)) {
	...
	$pdf->CellFitScale(70, 10, utf8_decode($municipio), 1, 0, 'C');
	...
	if($validacion=='ACTIVA'){
		$pdf->CellFitScale(25, 10, utf8_decode(date("H:i", strtotime($rhoraper))), 1, 0, 'C');
		$pdf->CellFitScale(20, 10, utf8_decode(date("H:i", strtotime($rhoracier))), 1, 0, 'C');
	$pdf->Ln();
	}else{
		$pdf->CellFitScale(45, 10, utf8_decode('CERRADA'), 1, 0, 'C');
		$pdf->Ln();
	}
}
